...
<nav class="menu" bind:this={menuElem}>
  <button class="btn--menu" onclick={toggleMenuBox}>
  ...
  {#if showMenuBox}
    ...
    <ClickOutside exclude={[menuElem]} onoutside={toggleMenuBox} />
    ...
    <ul class="menu__box" transition:fade>
      ...
      <button class="btn--create" onclick={showForm}>CRIAR NOVO QUADRO</button>
    </ul>
  {/if}
</nav>

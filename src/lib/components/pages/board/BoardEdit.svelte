<script>
  ...
  import { lists, boardBackground } from '$lib/store'
  import { generateLink } from '$lib/utils'
  import { BoardsDB, CardsDB, ListsDB } from '$lib/datastore'
  ...
  async function updateBoard () {
    ...
    await goto(generateLink(name, id))
  }
...
<aside class="board__edit edit" transition:fly={{ x: 150 }}>
  <header class="edit__header">
    ...
    <button class="icon-close" onclick={toggleEdit} aria-label="Fechar edição do quadro" title="Fechar edição do quadro"></button>
  </header>
  ...
  <div class="edit__form">
    ...
    <div class="edit__background">
      {#each $boardBackground as background, index}
        <button
          class={`btn--background ${background} ${selected == index ? 'selected' : ''}`}
          onclick={select(background)}
          id={index}
          aria-label={`Selecionar fundo ${background}`}
          title={`Selecionar fundo ${background}`}
        ></button>
      {/each}
    </div>

    <button class="btn--update btn btn--primary" onclick={updateBoard} disabled={!name}>ATUALIZAR</button>
  </div>

  <div class="edit__delete">
    <button class="btn--delete" onclick={deleteBoard}>EXCLUIR QUADRO</button>
  </div>
</aside>

<script>
  ...
  import { ListsDB, CardsDB } from '$lib/datastore'
...
<header class="list__header" bind:this={headerElem}>
  ...
  <nav class="list__menu" bind:this={navElement} >
    <button class="list__add icon-add" onclick={toggleFormCreateCard} aria-label="Adicionar cartão" title="Adicionar cartão"></button>
    <button class="list__options icon-options" onclick={toggleListOptions} aria-label="Abrir opções da lista" title="Abrir opções da lista"></button>
  </nav>
</header>
...
{#if showFormUpdateList}
  <div class="form__card" bind:this={formUpdateListElem} transition:fade>
    ...
    <input type="text" placeholder="Digite o nome do cartão"
      bind:value={name} onkeydown={(e) => e.which === 13 && updateList()} autofocus>

    <div>
      <button class="btn btn--primary" onclick={updateList} disabled={!name}>ATUALIZAR</button>
      <button class="btn--close icon-close" onclick={toggleFormUpdateList} aria-label="Fechar edição da lista" title="Fechar edição da lista"></button>
    </div>
  </div>
{/if}
...
{#if showListOptions}
  <div class="options" bind:this={listOptionsElem} transition:fade>
    <button class="options__edit" onclick={toggleFormUpdateList}>Editar Nome</button>
    <hr>
    <button class="options__delete" onclick={deleteList}>EXCLUIR LISTA</button>
  </div>
{/if}
